...
function calculateQuantities(ingredients, layers) {
  ...
  obj.sugar = ingredients.includes('sugar') ? 100 * layers : 0
  obj.eggs = ingredients.includes('eggs') ? 2 * layers : 0
...
function improveRecipe(recipeObj, portions) {
  const newRecipeObj = {}

  for (const key in recipeObj) {
    newRecipeObj[key] = recipeObj[key] * portions
  }
